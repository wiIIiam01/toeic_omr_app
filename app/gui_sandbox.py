...
class OMRLayoutDesign:
    ...
    def __init__(self, master: tk.Tk):
        ...
        try:
            ...
            messagebox.showerror("Lỗi Cấu hình Key", f"Không thể tải dữ liệu key từ {KEY_PATH}: {e}")
            print(f"Lỗi: Không thể tải dữ liệu key từ {KEY_PATH}: {e}")
            
        # --- 2. BIẾN TRẠNG THÁI (STATE VARIABLES) ---
        UNSELECTED_SET = "Select Set"
        UNSELECTED_ID = "Test"
        
        self.selected_set_var = tk.StringVar(master, value=UNSELECTED_SET)
        self.selected_id_var = tk.StringVar(master, value=UNSELECTED_ID) 
        self.test_date_var = tk.StringVar(master)
        
        self.has_files_loaded = False 
        self.key: str = "" # Biến để lưu chuỗi đáp án chuẩn hiện tại
        
        # Lưu giá trị mặc định cho Combobox để kiểm tra việc chọn lại giá trị ban đầu
        self.UNSELECTED_SET = UNSELECTED_SET
        self.UNSELECTED_ID = UNSELECTED_ID

        # --- 3. TẠO CÁC KHUNG (FRAMES) CHÍNH ---
    ...
    def _on_set_selected(self, event=None):
        # Lấy giá trị Set mới được chọn
        selected_set = self.selected_set_var.get()
                
        if selected_set and selected_set != self.UNSELECTED_SET:
            selected_set = self.selected_set_var.get()
            if selected_set == self.UNSELECTED_SET:
                return
                
            if selected_set and selected_set in self.all_keys_data:
                self._update_test_id_combo(selected_set)
                self.selected_id_var.set(self.UNSELECTED_ID)
                self.key = ""
    ...
    def _on_id_selected(self, event=None):
        ...
        # Ràng buộc: Không làm gì nếu Mã đề là UNSELECTED_ID
        if selected_id == self.UNSELECTED_ID:
            self.key = ""
            print("Mã đề chưa được chọn.")
            return
            
        if selected_set in self.all_keys_data and selected_id in self.all_keys_data[selected_set]:
            # Cập nhật chuỗi đáp án chuẩn vào biến self.key
            try:
                self.key = get_answer_key(self.all_keys_data, selected_set, selected_id)
            except Exception as e:
                messagebox.showerror("Lỗi Key", f"Không thể lấy đáp án chuẩn: {e}")
                self.key = ""
        else:
            self.key = "" # Đảm bảo key rỗng nếu không hợp lệ
            
        print(f"Selected Set: {self.selected_set_var.get()}, Selected ID: {self.selected_id_var.get()}, Key Status: {'Loaded' if self.key else 'Empty'}")


    def _update_test_id_combo(self, selected_set: str):
        # Vô hiệu hóa Mã đề Combobox trước khi cập nhật
        self.id_combo.config(state='disabled')
        self.id_combo.set(self.UNSELECTED_ID)
    ...
    def _create_top_controls(self):
        ...
        # Cập nhật values và thêm sự kiện cho Set Combobox
        initial_set_values = [self.UNSELECTED_SET] + self.set_names_list 
        self.set_combo = ttk.Combobox(input_area, textvariable=self.selected_set_var,
                                     values=self.set_names_list, # SỬA LỖI: Chỉ điền danh sách Set hợp lệ vào values
                                     state='readonly',
                                     )
        ...
        # Mã đề mặc định là list rỗng, vô hiệu hóa
    ...
    def _create_drag_drop_area(self):
        # Giữ nguyên _create_drag_drop_area
